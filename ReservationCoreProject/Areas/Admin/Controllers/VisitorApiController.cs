...
namespace ReservationCoreProject.Areas.Admin.Controllers
{
    ...
    public class VisitorApiController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:13858/api/Visitor");
        ...
        public async Task<IActionResult> AddVisitor(VisitorViewModel p)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:13858/api/Visitor", content);
            if(responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client=_httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:13858/api/Visitor/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client=_httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:13858/api/Visitor/{id}");
            if(responseMessage.IsSuccessStatusCode)
            {
                var jsonData=await responseMessage.Content.ReadAsStringAsync();
